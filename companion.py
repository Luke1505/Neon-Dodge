...
class Companion(pygame.sprite.Sprite):
    ...
    COMPANION_OFFSET_Y = 5    # Slightly below player's top
    ...
    def __init__(self, player_rect): # Pass the whole player rect for better positioning
        super().__init__()
        self.width = 20
        self.height = 20
        self.image = pygame.Surface([self.width, self.height], pygame.SRCALPHA)
        self.color = (100, 150, 255) # A distinct blue
        pygame.draw.rect(self.image, self.color, (0,0, self.width, self.height), border_radius=5)
    ...
    def shoot(self):
        ...
        new_bullet = Bullet(bullet_start_x, bullet_start_y, speed_y=-15, color=(180, 180, 255), radius=5)
        return [new_bullet] 

    def draw(self, screen): # Technically not needed if using sprite group's draw method for a single companion
